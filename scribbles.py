...
def delete(node, value):
    ...
    # problem: can't just delete a node
    # need to know its parent, and whether it is a left or right child
    # nodes don't currently point to their parents
    # so: easy enough to search a tree, not easy to delete the last node
    # store the last node that has children in a variable
    # the last child of the last node that has children is the last node, in level order
    # that's it
    q = deque()
    q.append(node)
    target = None
    last_parent = False
    # traverse in level order. aim to find node with specified value, the last node that has children
    ...
    if last_parent.right:
        ...
        # below operations only appropriate if target found
        if target:



            # simplest case: one child
            if front.left and not front.right:
                # front.left is the last node
                # store its data then remove pointer to it
                target.data = front.left.data
                front.left = None
                # operation complete
                return
            # next simplest case: node with no children
            if not front.left and not front.right:
                # right child of previous front is last node
                target.data = previous_front.right.data
                previous_front.right = None
                # fin
                return
        # continue traversal    
        if front.left:
        ...
        if front.right:
            q.append(front.right)
        previous_front = front
    # final case: all parents have two children
    # last node is right child of last node
    # target not found
    if not target:
        raise ValueError(f"{value} not in tree")